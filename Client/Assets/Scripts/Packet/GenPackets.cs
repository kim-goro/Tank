...
public class C_ConnectUDPPort : IPacket
{
	public int availablePort;

	public ushort Protocol { get { return (ushort)PacketID.C_ConnectUDPPort; } }
	...
	public void Read(ArraySegment<byte> segment)
	{
		...
		count += sizeof(ushort);
		count += sizeof(ushort);
		this.availablePort = BitConverter.ToInt32(segment.Array, segment.Offset + count);
		count += sizeof(int);
	}
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes((ushort)PacketID.C_ConnectUDPPort), 0, segment.Array, segment.Offset + count, sizeof(ushort));
		count += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes(this.availablePort), 0, segment.Array, segment.Offset + count, sizeof(int));
		count += sizeof(int);
